...
from widgets import AspectRatioWidget, ClickableImageViewer, HighlightDelegate, CheckableComboBox
from cache_utils import _to_relative, _game_cache_dir_for_game, _save_bytes_to_game_cache, scan_cache_directory_for_game, resolve_cover_art_cache_path
...
class GameManager(QMainWindow):
    ...
    def __init__(self):
        ...
        self._setup_trailer_player()        # <-- now uses the new method
    ...
    def _setup_data_model(self):
        ...
        # Helper to prepend config icon if available
        def col_header(label, icon_attr=None):
            if icon_attr and hasattr(config, icon_attr):
                icon = getattr(config, icon_attr)
                return f"{icon} {label}" if label else icon
            return label

        headers = [
            col_header("Title", None),
            col_header("Ver", "ICON_VERSION"),
            col_header("Drive", "ICON_DRIVE"),
            col_header("Steam", "ICON_STEAM"),
            col_header("", "ICON_PLAYED"),
            col_header("", "ICON_FAVOURITE"),
            col_header("Genres", "ICON_GENRES"),
            col_header("Modes", "ICON_MODES"),
            col_header("Release", "ICON_RELEASE"),
            col_header("Themes", "ICON_THEMES"),
            col_header("Dev", "ICON_DEVELOPER"),
            col_header("Pub", "ICON_PUBLISHER"),
            col_header("Scene/Repack", "ICON_SCENE"),
            col_header("Perspective", "ICON_PERSPECTIVE"),
            col_header("Original Title", "ICON_ORIGINAL"),
            col_header("IGDB", "ICON_IGDB"),
            col_header("Screenshots", "ICON_SCREENSHOTS"),
            col_header("Trailer (micro)", "ICON_TRAILER"),
            col_header("SteamDB", "ICON_LINKS"),
            col_header("PCGamingWiki", "ICON_LINKS"),
            col_header("Steam", "ICON_LINKS"),
            col_header("Description", None),
            col_header("IGDB Trailers", "ICON_IGDB_TRAILERS"),
            col_header("Cover URL", None),
            col_header("Extra microtrailers", "ICON_TRAILER"),
            col_header("Original Title Base", None),
            col_header("Original Notes", "ICON_NOTES"),  # fallback if not defined
            col_header("Image Cache Paths", None),
            col_header("Savegame Locations", "ICON_SAVE"),
            col_header("Microtrailers Cache Paths", None),
            col_header("User Rating", "ICON_USER_RATING"),
            col_header("IGDB CoverArt", None),
        ]
    ...
    def _setup_top_panel(self):
        ...
        # --- Search + Filters in one row (no labels) ---
        filter_widget = QWidget()
        filter_layout = QHBoxLayout(filter_widget)
        filter_layout.setContentsMargins(10, 10, 20, 5)
        ...
        # Add widgets with stretch factors: search gets more space
        filter_layout.addWidget(self.search, 2)
        filter_layout.addWidget(self.genre_filter, 1)
        filter_layout.addWidget(self.game_drive_filter, 1)

        left_layout.addWidget(filter_widget)


        self.search.setToolTip("Search all columns (case‑insensitive)")
        self.genre_filter.setToolTip("Click the dropdown to check one or more genres from your\n"
                                      "collection, or type directly to filter by any text (partial match).\n"
                                      "Multiple selections match games with ANY of the checked genres.")
        self.game_drive_filter.setToolTip("Click the dropdown to check one or more drives from your\n"
                                           "collection, or type directly to filter by any text (partial match).\n"
                                           "Multiple selections match games with ANY of the checked drives.")
        ...
        for stat in stats_config:
            ...
            # Use config colors for title and value
            scaled_stat_font = int(12 * config.GLOBAL_FONT_SCALE)
            title_label.setStyleSheet(f"font-size: {scaled_stat_font}px; font-weight: bold; color: {config.LABEL_TEXT_COLOR};")
            title_label.setAlignment(Qt.AlignCenter)
            title_label.setToolTip(card_tooltips.get(stat["id"], ""))
            value_label = QLabel("0")
            value_label.setStyleSheet(f"font-size: {scaled_stat_font}px; font-weight: bold; color: {config.UI_TEXT_COLOR};")            
    ...
    def refresh_ui(self):
        """Back-compat alias -- refresh entire UI after config changes."""
        self.apply_theme()

    def _refresh_stat_card_colors(self):
        """Update stat card left border using current config colors."""
        color_map = {
    ...
    def _setup_buttons(self):
        ...
        button_layout.setContentsMargins(10, 0, 0, 5)
        ...
        sep1 = QLabel("|")
        sep1.setStyleSheet(f"color: {config.BORDER_COLOR};")
        ...
        sep2 = QLabel("|")
        sep2.setStyleSheet(f"color: {config.BORDER_COLOR};")
        self.scrape_btn = QPushButton("🗲 Scrape Metadata")
        self.scrape_btn.clicked.connect(lambda: self.scrape_all(config.AUTO_ACCEPT_SCORE))
    ...
    def _setup_details_panel(self):
        ...
        self.details_title.setStyleSheet(f"font-size: {config.DETAILS_TITLE_FONT_SIZE}px; font-weight: bold; color: {config.UI_TEXT_COLOR}; padding: 4px;")
        details_layout.addWidget(self.details_title)

        # ----- Cover art: fixed 16:9 label -----
        # Base width chosen so that at default multiplier 1.5, cover is ~460px wide
        ...
        # ... rest of the method (external links, game info, etc.) unchanged ...

        # ----- External Links and Game Information (unchanged) -----
        ...
        self.details.setStyleSheet("QTextEdit { margin: 0; padding: 0; border: none; background: transparent; }")
    ...
    def _setup_image_viewer(self):
        ...
        # Use DARK_BG for the viewer background
        self.viewer.setStyleSheet(f"background-color: {config.DARK_BG}; border-radius: 4px; padding: 0px; margin: 0px;")
        ...
        scaled_btn_font = int(16 * config.GLOBAL_FONT_SCALE)
        btn_style = f"""
        QPushButton {{
        ...
        self.image_counter.setStyleSheet(f"background-color: rgba(0,0,0,160); color: {config.BUTTON_TEXT_COLOR}; padding: 4px 8px; border-radius: 3px;")
    ...
    def _setup_main_layout(self):
        ...
        # --- NEW: Remove margins from QTabWidget itself ---
        right_tabs.setDocumentMode(True)   # reduces default margins
    ...
    def resizeEvent(self, event):
        ...
        QTimer.singleShot(50, self._position_navigation_buttons)
        QTimer.singleShot(50, self._position_trailer_buttons)

        
    def clear_filters(self):
        """Clear search, genre, drive, and proxy filter."""
        self.search.clear()
        self.proxy.setFilterFixedString("")
        self.genre_filter.clear_all()
        self.game_drive_filter.clear_all()
    ...
    def _setup_trailer_player(self):
        ...
        self.trailer_nav_container = QWidget(self.trailer_container)
        self.trailer_nav_container.setStyleSheet("background: transparent;")
        self.trailer_nav_container.setAttribute(Qt.WA_TransparentForMouseEvents, False)

        # --- Scale the font and fix braces (use f-string) ---
        scaled_btn_font = int(16 * config.GLOBAL_FONT_SCALE)
        btn_style = f"""
        QPushButton {{
            background-color: rgba(0,0,0,180);
        ...
        self.open_trailer_btn = QPushButton("🌐", self.trailer_nav_container)
        self.open_trailer_btn.setFixedSize(40, 40)
        self.open_trailer_btn.setEnabled(False)
        self.open_trailer_btn.setToolTip("Open trailer in browser")
        self.open_trailer_btn.setStyleSheet(btn_style)
        self.open_trailer_btn.clicked.connect(self._open_trailer_in_browser)

        self.expand_trailer_btn = QPushButton("⛶", self.trailer_nav_container)
        self.expand_trailer_btn.setFixedSize(40, 40)
        self.expand_trailer_btn.setEnabled(False)
        self.expand_trailer_btn.setToolTip("Open fullscreen viewer")
        self.expand_trailer_btn.setStyleSheet(btn_style)
        self.expand_trailer_btn.clicked.connect(lambda: self._open_media_viewer("video"))

        self.trailer_container.resizeEvent = self._on_trailer_container_resize
        QTimer.singleShot(100, lambda: self.trailer_nav_container.raise_())

    def _on_trailer_container_resize(self, event):
        QWidget.resizeEvent(self.trailer_container, event)
        QTimer.singleShot(50, self._position_trailer_buttons)

    def _position_trailer_buttons(self):
        try:
            if not hasattr(self, 'open_trailer_btn'):
                return
            rect = self.trailer_container.rect()
            if rect.isEmpty():
                return
            self.trailer_nav_container.setGeometry(rect)
            button_y = max(10, rect.height() - 60)
            center_x = (rect.width() - 40) // 2
            self.open_trailer_btn.move(center_x - 50, button_y)
            self.expand_trailer_btn.move(center_x + 50, button_y)
            self.trailer_nav_container.raise_()
        except Exception as e:
            print(f"Trailer button positioning error: {e}")
        ...
        if col == self.COL_PLAYED:
            ...
            if title_item:
                ...
                if self.games[row].get("played"):
                    sym.append(config.ICON_PLAYED)
                if self.games[row].get("fav"):
                    sym.append(config.ICON_FAVOURITE)
            ...
            for game_idx, game in enumerate(self.games):
                ...
                for col in range(len(self.COLUMN_KEYS)):
                    ...
                    if col == 0:  # Title column
                        # Example in refresh_model
                        title_text = game.get("title", "")
                        if game.get("played"):
                            title_text = f"{title_text} {config.ICON_PLAYED}"
                        if game.get("fav"):
                            title_text = f"{title_text} {config.ICON_FAVOURITE}"
                        value = title_text
    ...
    def apply_theme(self):
        ...
        change to every custom child widget (combo boxes, image viewers,
        etc.) automatically -- nothing needs manual wiring here.
        """
        if config.AUTO_SAVE:
        ...
        # Reapply UI colours (palette) and the full stylesheet
        ...
        # Local (non-widget-owned) pieces this window is responsible for
        self._refresh_stat_card_colors()
        self._apply_table_font()

        # Propagate to every child widget that defines apply_theme()
        # (CheckableComboBox filters, image viewers, etc.)
    ...
    def _apply_ui_colors(self):
        ...
        palette.setColor(QPalette.Button, QColor(config.BUTTON_BACKGROUND))
        ...
        palette.setColor(QPalette.WindowText, QColor(config.UI_TEXT_COLOR))
    ...
    def scrape_all(self, auto_accept_score=None):
    ...
    def _get_thumbnail_data_uri(self, game: dict) -> str:
        """Return a data URI for the cover art, preferring the Steam header image."""
        try:
            ...
            # 1. Try Steam header image (if app_id exists)
            app_id = game.get("app_id", "")
            if app_id and str(app_id).strip():
                steam_header_url = f"https://steamcdn-a.akamaihd.net/steam/apps/{app_id}/header.jpg"
                candidate = resolve_cover_art_cache_path(game, steam_header_url)
            ...
            # 2. Fallback: existing cover_url / igdb_cover_art (as before)
            cover_url = game.get("cover_url", "") or game.get("igdb_cover_art", "")
            if cover_url:
                candidate = resolve_cover_art_cache_path(game, cover_url)
                if candidate and candidate.exists() and candidate.stat().st_size > 0:
                    with open(candidate, "rb") as f:
                        img_data = base64.b64encode(f.read()).decode('utf-8')
                    ext = candidate.suffix.lower()
            ...
            # 3. Last resort: any valid image >20KB in cache
   ...
   # ----------------------------------------------------------------------
    ...
    def show_details_for_source_row(self, source_row: int):
        ...
        self._current_details_row = source_row

        # ---- 1. Title (with rating, symbols) ----
        title_text = game.get("title", "Untitled")
        symbols = []
        if game.get("played"):
            symbols.append(config.ICON_PLAYED)
        if game.get("fav"):
            symbols.append(config.ICON_FAVOURITE)
        ...
        if user_rating:
            try:
                ...
                clamped = max(0, min(100, rating_num))
                stars = clamped / 20.0
                full = int(stars)
                half = 1 if (stars - full) >= 0.5 else 0
                empty = 5 - full - half
                stars_str = "★" * full + "½" * half + "☆" * empty
                display = f"{int(rating_num)}" if rating_num == int(rating_num) else f"{rating_num:.1f}"
                rating_str = f'<span style="color:{config.DETAILS_RATING_COLOR}; font-size:{config.DETAILS_RATING_FONT_SIZE}px;">{stars_str}</span> <span style="color:{config.LABEL_TEXT_COLOR};">({display})</span>'
            except:
                rating_str = " <span style='color:#f39c12;'>☆</span> <span style='color:#555;'>(No rating)</span>"

        full_title = f"{title_text}{symbols_html} {rating_str}"
        self.details_title.setText(full_title)

        # ---- 2. Cover image (load asynchronously to avoid UI freeze) ----
        # Clear current cover and show loading text
        # ---- 2. Cover image (load asynchronously) ----
        self._load_cover_for_game(game)


        # ---- 3. Build the HTML for the rest (description + tables) ----
        def escape_html(t):
            return (t.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;").replace("\n","<br>"))

        desc = game.get("description", "").strip()
        desc_html = f'<div style="margin:0 0 6px 0; max-height:150px; overflow-y:auto; color:{config.DETAILS_DESCRIPTION_COLOR};">{escape_html(desc) or "<i>No description</i>"}</div>'

        basic_fields = [
            (f"{config.ICON_RELEASE} Release date", game.get("release_date", "")),
            (f"{config.ICON_GENRES} Genres", game.get("genres", "")),
            (f"{config.ICON_MODES} Game modes", game.get("game_modes", "")),
            (f"{config.ICON_THEMES} Themes", game.get("themes", "")),
        ]
        technical_fields = [
            (f"{config.ICON_VERSION} Version", game.get("patch_version", "") or game.get("original_title_version", "")),
            (f"{config.ICON_DRIVE} Game Drive", game.get("game_drive", "")),
            (f"{config.ICON_STEAM} Steam ID", game.get("app_id", "")),
            (f"{config.ICON_IGDB} IGDB ID", game.get("igdb_id", "")),
            (f"{config.ICON_SCENE} Scene/Repack", game.get("scene_repack", "")),
            (f"{config.ICON_PERSPECTIVE} Perspective", game.get("player_perspective", "")),
            (f"{config.ICON_DEVELOPER} Developer", game.get("developer", "")),
            (f"{config.ICON_PUBLISHER} Publisher", game.get("publisher", "")),
            (f"{config.ICON_ORIGINAL} Original title", game.get("original_title", "")),
        ]
        ...
        def build_table(fields, title):
            ...
            for label, val in fields:
                ...
                rows.append(f'''<tr style="border-bottom:1px solid {config.BORDER_COLOR};">
                    <td style="padding:6px 8px; width:130px; font-weight:600; color:{config.DETAILS_METADATA_LABEL_COLOR}; font-size:{config.DETAILS_METADATA_LABEL_FONT_SIZE}px;">{label}</td>
                    <td style="padding:6px 8px; color:{config.DETAILS_METADATA_VALUE_COLOR}; font-size:{config.DETAILS_METADATA_VALUE_FONT_SIZE}px;">{cell}</td>
                </tr>''')
            if not rows:
                return ""
            return f'<h4 style="margin:12px 0 6px 0; color:{config.UI_TEXT_COLOR}; font-size:{config.DETAILS_SECTION_HEADER_FONT_SIZE}px;">{title}</h4><table style="width:100%; border-collapse:collapse;">{"".join(rows)}</table>'
        ...
        locs_html = ""
        if save_locs:
            locs_html = f'<h4 style="margin:12px 0 6px 0; color:{config.UI_TEXT_COLOR}; font-size:{config.DETAILS_SECTION_HEADER_FONT_SIZE}px;">{config.ICON_SAVE} Savegame Locations</h4><div style="background:{config.LIGHT_BG}; border-radius:6px; padding:8px;">'
            for loc in save_locs:
                try:
                    href = QUrl.fromLocalFile(str(loc)).toString() if not loc.startswith(("http://","https://")) else loc
                    locs_html += f'<div style="margin:4px 0;">📁 <a href="{href}" style="color:{config.ACCENT_COLOR}; text-decoration:none;">{escape_html(str(loc))}</a></div>'
                except:
                    locs_html += f'<div>📁 {escape_html(str(loc))}</div>'
            locs_html += '</div>'

        # Combine everything except cover (which is now separate)
        combined = f'''
        <div style="font-family: 'Segoe UI', Arial, sans-serif; font-size:{config.DETAILS_DESC_FONT_SIZE}px; line-height:1.3;">
            {desc_html}
        ...
        for link_name, link_key in [("Steam","steam_link"), ("SteamDB","steamdb_link"), ("PCGamingWiki","pcgw_link"), ("IGDB","igdb_link")]:
            ...
            if url:
                ...
                links.append(f'<a href="{full_url}" style="color:{config.ACCENT_COLOR}; text-decoration:underline; margin-right:12px;">{link_name}</a>')
        ...
        # ---- 5. Image viewer / trailer (unchanged) ----
        # ... (keep the existing code for image display and trailer player)
        # Ensure we still call image_display.fetch_and_display_images and trailer_player
        ...
        # In show_details_for_source_row
        trailer_url = self._get_current_trailer_url(game)
        if trailer_url:
            self.trailer_container.show()
            self.trailer_player.play_trailer_media(trailer_url, game)   # <-- pass game
        else:
            self.trailer_container.hide()

        if hasattr(self, "open_trailer_btn"):
            self.open_trailer_btn.setEnabled(bool(trailer_url))
            self.expand_trailer_btn.setEnabled(bool(trailer_url))

    def _get_current_trailer_url(self, game=None):
        if game is None:
            row = getattr(self, "_current_details_row", None)
            if row is None or row < 0 or row >= len(self.games):
                return ""
            game = self.games[row]
    ...
    def _open_trailer_in_browser(self):
        trailer_url = self._get_current_trailer_url()
        if not trailer_url:
            return
        try:
    ...
    def _open_media_viewer(self, media_type=None):
        """Open the same fullscreen media viewer (ScreenshotViewerDialog)
        used by the details panel, pre-loaded with the current game's
        trailer and screenshots. By default starts at the image currently
        shown; pass media_type="video" to start on the trailer instead."""
        row = getattr(self, "_current_details_row", None)
        if row is None or row < 0 or row >= len(self.games):
            return
        game = self.games[row]
        ...
        trailer_url = self._get_current_trailer_url(game)
        if trailer_url:
            media_items.append({"type": "video", "url": trailer_url})
        ...
        if media_type == "video" and trailer_url:
            start_index = 0
        else:
            cur_img_idx = getattr(self.image_display, "_current_image_index", 0)
            start_index = cur_img_idx + (1 if trailer_url else 0)
    ...
    def _collect_unique_filter_values(self, game_key: str) -> list:
        """Gather every distinct value currently present in self.games for
        a given field, for populating a CheckableComboBox's checklist.
        Splits comma-joined multi-value fields (genres, themes, etc.) into
        individual tokens so e.g. "Action, Adventure" contributes two
        separate checkable entries rather than one combined string;
        harmless no-op for single-value fields like game_drive."""
        values = set()
        for g in self.games:
            raw = g.get(game_key, "")
    ...
    def _refresh_filter_options(self):
        """Repopulate the Genre/Drive checklist dropdowns from current
        data. Safe to call any time (e.g. every refresh_model()) - checkbox
        states are derived from whatever's currently typed/checked, so this
        never clobbers the user's active filter selection."""
    ...
    def apply_filters(self):
        # currentText() reads the combo's line edit, which holds whatever
        # combination of checked items + manually typed text is currently
        # there. Split on "," for OR-matching multiple selections; a single
        # typed term with no comma behaves exactly like the old plain
        # substring filter.
        genre_terms = [t.strip().lower() for t in (self.genre_filter.currentText() or "").split(",") if t.strip()]
        drive_terms = [t.strip().lower() for t in (self.game_drive_filter.currentText() or "").split(",") if t.strip()]
        if not genre_terms and not drive_terms:
        ...
        for r in range(self.proxy.rowCount()):
            ...
            if src.isValid():
                ...
                if genre_terms and not any(t in genre for t in genre_terms):
                    show = False
                if drive_terms and not any(t in drive for t in drive_terms):
                    show = False
            else:
                show = False
    ...
    def open_context_menu(self, pos):
        ...
        rows = self._selected_source_rows()
        if rows:
            fav_action = QAction(f"{config.ICON_FAVOURITE} Add to Favourite", self)
            fav_action.triggered.connect(self.toggle_favourite_selected)
            menu.addAction(fav_action)
            played_action = QAction(f"{config.ICON_PLAYED} Toggle Played Status", self)
            ...
            if meta and "__candidates__" not in meta:
                ...
                if game.get('title') != manual_title:
                    ...
                    if title_item:
                        ...
                        if game.get('played'):
                            title_text += f" {config.ICON_PLAYED}"
                        if game.get('fav'):
                            title_text += f" {config.ICON_FAVOURITE}"
    ...
    def build_menus(self):
        ...
        edit_menu.addSeparator()
        toggle_played = QAction(f"{config.ICON_PLAYED} Toggle Played Status", self)
        ...
        toggle_fav = QAction(f"{config.ICON_FAVOURITE} Toggle Favourite", self)
        ...
        scrape_action.triggered.connect(lambda: self.scrape_all(config.AUTO_ACCEPT_SCORE))
    ...
    def test_scrape_single(self):
        ...
        try:
            meta = scraping.scrape_igdb_then_steam(None, title, auto_accept_score=config.AUTO_ACCEPT_SCORE, fetch_pcgw_save=False) or {}
            if meta:
